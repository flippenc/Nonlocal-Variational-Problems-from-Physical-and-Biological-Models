function genModel = twoCirclesParameterizedGenModel(alpha,beta,gamma,delta,epsilon,lambda,N,tMax)
...
xc = -sqrt(epsilon/4);
yc = 0;
theta = rand(1,0.5*N)*(2*pi);
r = sqrt(rand(1,0.5*N))*radius;
x = xc + r.*cos(theta);
y = yc + r.*sin(theta);
r(1:0.5*N,1)=x;
r(1:0.5*N,2)=y;

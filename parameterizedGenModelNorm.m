function genModel = parameterizedGenModelNorm(alpha,beta,gamma,delta,epsilon,lambda,a,b,c,N,tMax)
...
kernelParamList = strcat('alpha=',num2str(alpha),',beta=', ...
    ...
    num2str(a),',b=',num2str(b),',c=',num2str(c)');
...
tspan = [0 tMax];
sol = ode45(@(t,y) normParameterizedSystemGrad(t, y, alpha, beta, gamma, delta, epsilon, lambda, a, b, c, N), tspan, y0);
...
for i=0:m
    ...
    if ~(any(isnan(xcol)) || any(isnan(ycol)))        
        ...
        ellipticalText = strcat('$(ax)^c + (by)^c =(',num2str(a),'x)^',strcat(num2str(c)), ...
            ' + (',num2str(b),'y)^',strcat(num2str(c)),'$');
        ...
        axis equal
        axis on
        title({paramText ellipticalText distText timeText},'Interpreter','latex')
        print('-dpng','-r150', ...
